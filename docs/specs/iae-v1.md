...
1. As a visitor, I want to sign in with Google or an email magic link, so that I can access my projects securely.
...
- **Auth**: Clerk (Google + email magic link only). Backend verifies JWT on every HTTP request and WebSocket handshake. Lazy **User** upsert in auth middleware (no dedicated POST endpoint). Frontend calls `GET /api/users/me` when Clerk session becomes active.
